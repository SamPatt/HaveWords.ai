...
async function triggerImageBot(response) {
  ...
  if (!apiKey) {
    ...
    AiChatView.shared().addMessage("systemMessage", "API key is missing.", "System", LocalUser.shared().id());
    return;
  }
  const message =
...
async function triggerBot(response, sessionType, sessionDetails) {
  ...
  if (!apiKey) {
    console.error("API key is missing.");
    AiChatView.shared().addMessage("systemMessage", "API key is missing.", "System");
    return;
  }
